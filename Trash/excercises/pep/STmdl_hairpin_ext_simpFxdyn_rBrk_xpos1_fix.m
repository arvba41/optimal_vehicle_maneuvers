...
% load('interesting_results\hrp_50m_30kmph_t_opt_init_res_v2.mat');
...
opti.subject_to(-Dxf<=Fxrval<=0);      % rear wheel braking only
opti.subject_to(-Dxf<=Fxf<=Dxf);    
opti.subject_to(-Dxf<=Fxr<=0);      % rear wheel braking only
