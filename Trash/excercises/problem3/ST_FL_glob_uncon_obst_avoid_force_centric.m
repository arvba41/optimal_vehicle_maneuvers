...
Fyf = @(vx,vy,r,delta,Fxf) Fy0f(vx,vy,r,delta,Fxf).*sqrt(1 - (Fxf./Dxf).^2);
Fyr = @(vx,vy,r,delta,Fxf) Fy0r(vx,vy,r,delta,Fxf).*sqrt(1 - (Fxf./Dxf).^2);
...
% ---- force constrains ----
opti.subject_to(-Dxf*0.98<=Fxf<=Dxf*0.98);  
...
% (use for N = 100)
idx_int = find(round(yobst_sim,5)== round(ypos_sim,5));
idx_int(1) = []; % ignoting the first element

if length(idx_int) ~= 1
    theta_sim = atan(diff(ypos_sim(idx_int))/diff(xpos_sim(idx_int)));
else
    theta_sim = atan(diff([ypos_sim(idx_int+1), ypos_sim(idx_int-1)])/diff([xpos_sim(idx_int+1), xpos_sim(idx_int-1)]));
end
...
Fxmax = max([Dxf Dxr]);
Fymax = max([Dyf Dyr]);
prob1.FPxmax = Fxmax.*cos(prob1.psi(2:end)) - Fymax.*sin(prob1.psi(2:end));
prob1.FPymax = Fxmax.*sin(prob1.psi(2:end)) + Fymax.*cos(prob1.psi(2:end));
...
%% Force centric approach

% ----- compute θv for the avoidance situation
% finding intersection point between vehciel and obstacle 
yobst_sim = obst(prob1.x,Xfin,Owdt,Osrp,Ohgt,Pwid);
ypos_sim = prob1.y;
xpos_sim = prob1.x;
...
% use the coordinate systems
% Earth fixed forces
prob1.FPx = prob1.F_X.*cos(prob1.psi(2:end)) - prob1.F_Y.*sin(prob1.psi(2:end));
prob1.FPy = prob1.F_X.*sin(prob1.psi(2:end)) + prob1.F_Y.*cos(prob1.psi(2:end));

% scenario centric 
prob1.Fcx = prob1.FPx.*cos(theta_sim) + prob1.FPy.*sin(theta_sim);
prob1.Fcy = -prob1.FPx.*sin(theta_sim) + prob1.FPy.*cos(theta_sim);
prob1.Fcxmax = prob1.FPxmax.*cos(theta_sim) + prob1.FPymax.*sin(theta_sim);
prob1.Fcymax = -prob1.FPxmax.*sin(theta_sim) + prob1.FPymax.*cos(theta_sim);
...
nexttile;
plot(prob1.tvec(2:end),prob1.Fcy);
ylabel('$F_{c,y}$ [N]','Interpreter','latex')
xlabel('$t$ [s]')
xline(mean(prob1.tvec(idx_int)),'--');
% yline(prob1.Fcymax,'--'); 

nexttile;
plot(prob1.tvec(2:end),prob1.Fcx);
ylabel('$F_{c,x}$ [N]','Interpreter','latex')
xlabel('$t$ [s]','Interpreter','latex')
xline(mean(prob1.tvec(idx_int)));
% yline(prob1.Fcxmax,'--'); 
...
nexttile;
plot(prob1.tvec(2:end),prob1.FPx);
ylabel('$F_{p,x}$ [N]','Interpreter','latex')
xlabel('$t$ [s]','Interpreter','latex')
xline(mean(prob1.tvec(idx_int)),'--');
% yline(prob1.FPxmax,'--'); 
