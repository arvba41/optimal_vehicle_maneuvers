...
% beta_y = opti.variable(); % final distance slack
mu = opti.variable();      % friction

% ---- objective          ---------
% opti.minimize(T + beta_y*0.1);         % minimize time
...
% opti.subject_to(0 <= xpos <= 2*Xfin);

% % ---- final distance slack constraint ----
% opti.subject_to(R2 - beta_y <= ypos(end) <= R2 - beta_y)
% opti.subject_to(-R2 <= beta_y <= R2);
...
% opti.subject_to(ypos(N+1)==R2);  % finish line at position y 
