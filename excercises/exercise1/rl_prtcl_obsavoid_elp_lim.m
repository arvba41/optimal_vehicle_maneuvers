function [oplsol,elps] = rl_prtcl_obsavoid_elp_lim(mdl,obst,cons,optops)
...
opti.subject_to(xpos(N+1)==Xfin);  % finish line at position x 1
% opti.subject_to(ypos(N+1)==ypos(1));  % finish line at position x 1
opti.subject_to(xspeed(1)==cons.vxinit);   % start with speed x 0 ...
...
opti.set_initial(xspeed, cons.vxinit);
...
oplsol.delta = sol.value(delta); oplsol.ddelta = sol.value(ddelta);
