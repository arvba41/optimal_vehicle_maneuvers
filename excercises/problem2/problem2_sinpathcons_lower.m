...
prob2.x = sol.value(xpos); prob2.y = sol.value(ypos);
prob2.vx = sol.value(xspeed); prob2.vy = sol.value(yspeed);
prob2.ay = sol.value(delta); prob2.jy = sol.value(ddelta);
prob2.ux = sol.value(U(1,:)); prob2.uy = sol.value(U(2,:));
prob2.topt = sol.value(T); prob2.tvec = linspace(0,sol.value(T),N+1);

prob2.cons.Flim = mass*mu*g;
%% plotting
%%%% debugging
prob2.x = opti.debug.value(xpos); prob2.y = opti.debug.value(ypos);
prob2.vx = opti.debug.value(xspeed); prob2.vy = opti.debug.value(yspeed);
prob2.ay = opti.debug.value(delta); prob2.jy = opti.debug.value(ddelta);
prob2.ux = opti.debug.value(U(1,:)); prob2.uy = opti.debug.value(U(2,:));
prob2.topt = opti.debug.value(T); prob2.tvec = linspace(0,opti.debug.value(T),N+1);

prob2.cons.Flim = mass*mu*g; 
prob2.cons.yllim = y1curve(prob2.x); 
% prob2.cons.yulim = yucurve(prob2.x);

f = figure(2); clf;
optim_plot_prob2(prob2);
