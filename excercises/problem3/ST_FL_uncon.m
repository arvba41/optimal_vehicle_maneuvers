...
% load('interesting_results\hrpo6_55m_30kmph_t_opt_free_vx_xpos.mat');
...
pow = 6;    % squareness of the obsticle 
...
opti.subject_to(3 <= xpos(1) <= 8);
% opti.subject_to((3 - betax) <= xpos(1) <= (3 + betax));
opti.subject_to(12 <= xpos(N+1) <= 17);
% opti.subject_to((Yfin - betay) <= ypos(N+1) <= (Yfin + betay));

% opti.subject_to(0 <= betax <= 0.5);
% opti.subject_to(0 <= betay <= 1);

% ---- boundary conditions --------
% define all the initial values here
% position boundry conditions (start)
% opti.subject_to(xpos(1)==3);        
...
% ---- force constrains ----
opti.subject_to(-Dxf*0.98<=Fxfval<=Dxf*0.98);    
opti.subject_to(-Dxf*0.98<=Fxrval<=0);      % rear wheel braking only
opti.subject_to(-Dxf*0.98<=Fxf<=Dxf);    
opti.subject_to(-Dxf*0.98<=Fxr<=0);      % rear wheel braking only
...
%%%% NaN error debugging 
opti.debug.g_describe(148)
opti.debug.x_describe(148)
...
% ---- selip (1/2)
fimplicit(@(x,y) path_in(x,y) - 1 ,[0 2*Xfin 0 Ohgt+Owdty],'--','Color','k');
fimplicit(@(x,y) path_out(x,y) - 1 ,[0 2*Xfin 0 Ohgt+Owdty],'--','Color','k')
