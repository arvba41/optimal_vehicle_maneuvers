...
% ---- intermediate equations
    ...
    FY = @(d,Fxf,Fyf) ...
          Fyf*cos(d) ...
        + Fxf*sin(d) ...
        + Fyf; % Fxr = Fxf %% disregarding the lateral slip 
...
% u(3) --> Fy (force in the y direction, at the front wheel)
% ---- ODEs ----
f = @(x,u) [ ... the posistion dynamics ...
                    x(3);...                    \dot x = vx
                    x(4);...                    \dot y = vy
                ... the speed dynamics ...
                    FX(u(1),u(2),u(3))/m + x(4)*x(5);...  \dot vx = FX/m + vy*yawrate
                    FY(u(1),u(2),u(3))/m - x(3)*x(5);...  \dot vx = FY/m - vx*yawrate
                    MZ(u(1),u(2),u(3))/Izz;...           \dot yawrate = MZ/Izz
                    ]; 
...
Dxmin = min([Dxf Dxr]);
Dymin = min([Dyf Dyr]); 
Fxmax = Dxmin/2; % maximum force x direction
Fymax = Dymin/2; % maximum force y direction
delta_max = 0;

Hsigmoid = @(x) 0.5*(1+tanh(2*pi*6*(x)/(10)));
Ppulse = @(x,puls_strt,puls_end) Hsigmoid(x-puls_strt) + Hsigmoid(puls_end-x) - 1;

Fxin = @(x,puls_strt,puls_end) Fxmax*cos(2*pi*(x)/10).*Ppulse(x,puls_strt,puls_end);
Fyin = @(x,puls_strt,puls_end) Fymax*sin(2*pi*(x)/10).*Ppulse(x,puls_strt,puls_end);
...
plot(Fxin(x,D_strt,D_end),Fyin(x,D_strt,D_end),'x-','LineWidth',2); box off 
xlim([-Fxmax Fxmax]); ylim([-Fxmax Fxmax])

ax(2) = subplot(2,2,3);
plot(x,Fxin(x,D_strt,D_end),'LineWidth',2); box off 

ax(3) = subplot(2,2,4);
plot(x,Fyin(x,D_strt,D_end),'LineWidth',2); box off 

tspan = [0 50];
y0 = zeros(5,1);
y0(3) = 0.1; y0(4) = 0;
[t,y] = ode15s(@(t,y) f(y,[deltain(t,D_strt,D_end), Fxin(t,D_strt,D_end) Fxin(t,D_strt,D_end)]),...
    tspan, y0);

potting_simresults_forceXY(t,y,11,params);
%
%% --- simulation number 1 ---
% No steering angle only positive wheel torque
Fxfmax = 1e3/2; % maximum force
...
potting_simresults_forceX(t,y,1,params,[D_strt D_end delta_max Fxf_strt Fxf_end Fxfmax]);
