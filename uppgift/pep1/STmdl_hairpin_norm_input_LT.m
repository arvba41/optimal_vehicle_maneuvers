...
% load('STmdl_smpl\topt\hrpin_10m_30kmph_init.mat');
...
% Fxfval = opti.variable(1,N);      % final time
% Fxrval = opti.variable(1,N);      % final time
...
% %----------- elipses 
path_in = @(x,y) (x/R1i).^n + (y/R2i).^n;
path_out = @(x,y) (x/R1o).^n + (y/R2o).^n;
...
opti.subject_to(-5 <= uf <= 5);    
opti.subject_to(-5 <= ur <= 0);      % rear wheel braking only
...
% % % % 
% % % % opti.set_initial(uf, prob1.uf);
% % % % opti.set_initial(ur, prob1.ur);
...
opti.set_initial(psi, pi/2);

opti.set_initial(T, 2.4);

opti.set_initial(uf, 1);
opti.set_initial(ur, 0);

opti.set_initial(Fxf, 5);
...
prob1.Fxfval = opti.debug.value(uf);
prob1.Fxrval = opti.debug.value(ur);
...
prob1.Fxfval = sol.value(uf);
prob1.Fxrval = sol.value(ur);
...
% fimplicit(@(x,y) path_in(x,y) - 1 ,[-R1o R1o 0 R2o],'--','Color','k');
% fimplicit(@(x,y) path_out(x,y) - 1 ,[-R1o R1o 0 R2o],'--','Color','k')
...
% using sigmoids to normalize the inputs 
Fxfval = Dxf * tanh(uf);
Fxrval = Dxr * tanh(ur);
...
Fyf = Fy0f.*sqrt(1.05 - (Fxf/Dxf)^2);
Fyr = Fy0r.*sqrt(1.05 - (Fxr/Dxr)^2);
