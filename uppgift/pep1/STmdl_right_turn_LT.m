...
% load('ST_right_turn\ST_LT_build\right_turn_50m_straight.mat');
load('solved_sol_LT.mat');
% load('temp_sol_LT.mat');
...
% vehicle parameters
params.m = 2100;    % original: 2100 ALT1: 2500
params.lf = 1.3;    % original: 1.3 
params.lr = 1.5;    % original: 1.5
params.g = 9.82;
params.Izz = 3900;  % original: 3900 (assuming linear relationship between Izz and m) ALT1: params.m * 3900 / 2100
params.Iw = 4;
params.Re = 0.3;
params.h = 0.50;
...
Fz = opti.variable(2,N+1);% normal force on the wheel      
Fzf     = Fz(1,:);
Fzr     = Fz(2,:);
...
for k=1:N % loop over control intervals 
   ...
   k1 = ST_model(X(:,k),         U(:,k),    params, Fz(:,k));
   k2 = ST_model(X(:,k)+dt/2*k1, U(:,k),    params, Fz(:,k));
   k3 = ST_model(X(:,k)+dt/2*k2, U(:,k),    params, Fz(:,k));
   k4 = ST_model(X(:,k)+dt*k3,   U(:,k),    params, Fz(:,k));
   x_next = X(:,k) + dt/6*(k1+2*k2+2*k3+k4); 
   opti.subject_to(X(:,k+1)==x_next); % close the gaps
   
   % load balancing 
   [~,~,Fx] = ST_model(X(:,k+1), U(:,k),  params, Fz(:,k));
   opti.subject_to(Fz(:,k+1) == [...
       (params.m*params.g*params.lr - params.h * Fx)/(params.lf + params.lr); ...
       (params.m*params.g*params.lf + params.h * Fx)/(params.lf + params.lr)  ...
       ]);
%    opti.subject_to(Fzr(k+1) == ...
%        (params.m*params.g*params.lf )/(params.lf + params.lr));
end
...
opti.subject_to(Fzf(1)== params.m * params.g * params.lr / (params.lf + params.lr));   
opti.subject_to(Fzr(1)== params.m * params.g * params.lf / (params.lf + params.lr));          
...
Dxf = params.mux(1) * Fzf; 
Dxr = params.mux(2) * Fzr;
opti.subject_to(-Dxf(2:end)<=Fxfval<=Dxf(2:end));    
opti.subject_to(-Dxr(2:end)<=Fxrval<=0);      % rear wheel braking only
...
opti.set_initial(Fzf, prob1.Fzf);
opti.set_initial(Fzr, prob1.Fzr);
...
% % % % 
% % % % opti.set_initial(Fzf, params.m * params.g * params.lr / (params.lf + params.lr));   
% % % % opti.set_initial(Fzr, params.m * params.g * params.lf / (params.lf + params.lr));  

% ---- maximum iterations ----
p_opts = struct('expand',true);
s_opts = struct('max_iter',20000);
...
prob1.Fzf = opti.debug.value(Fzf);
prob1.Fzr = opti.debug.value(Fzr);
                                        
for ii = 1 : (N+1)
    [~, inter(ii),Fxv(ii)] = ST_model(opti.debug.value(X(:,ii)), ...
                            opti.debug.value(U(:,1)), ...
                            params, ...
                            opti.debug.value(Fz(:,ii)));
...
Dxf = params.mux(1) * opti.debug.value(Fzf); 
Dxr = params.mux(2) * opti.debug.value(Fzr);
Dyf = params.muy(1) * opti.debug.value(Fzf); 
Dyr = params.muy(2) * opti.debug.value(Fzr);

prob1.topt = opti.debug.value(T); prob1.tvec = linspace(0,opti.debug.value(T),N+1);
%%%% NaN error debugging 


save('temp_sol_LT.mat','prob1')
%% post solution plots
...
prob1.Fzf = sol.value(Fzf);
prob1.Fzr = sol.value(Fzr);
...
Dxf = params.mux(1) * sol.value(Fzf); 
Dxr = params.mux(2) * sol.value(Fzr);
Dyf = params.muy(1) * sol.value(Fzf); 
Dyr = params.muy(2) * sol.value(Fzr);

prob1.topt = sol.value(T); prob1.tvec = linspace(0,sol.value(T),N+1);

save('solved_sol_LT.mat','prob1');
...
function [xdot, inter, Fx] = ST_model(x ,u  ,params ,Fz)
...
Fyf = Fy0f.*sqrt(1.01 - (min(max(Fxf,-Dxf),Dxf)/Dxf).^2);
Fyr = Fy0r.*sqrt(1.01 - (min(max(Fxr,-Dxr),Dxr)/Dxr).^2);
