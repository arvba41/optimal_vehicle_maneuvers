...
Fp = opti.variable(2,N);  % global forces  
Fpx     = Fp(1,:);
Fpy     = Fp(2,:);

Fc = opti.variable(2,N);  % control forces  
Fcx     = Fc(1,:);
Fcy     = Fc(2,:);
...
for k=1:N % loop over control intervals
   ...
   k1 = DT_model(X(:,k),         U(:,k),    params);
   k2 = DT_model(X(:,k)+dt/2*k1, U(:,k),    params);
   k3 = DT_model(X(:,k)+dt/2*k2, U(:,k),    params);
   k4 = DT_model(X(:,k)+dt*k3,   U(:,k),    params);
   x_next = X(:,k) + dt/6*(k1+2*k2+2*k3+k4); 
   opti.subject_to(X(:,k+1)==x_next); % close the gaps
   
   % get the forces
   [~, ~, Fx, Fy] = DT_model(X(:,k), U(:,k), params);
   
   % global forces
   opti.subject_to(Fpx(k) == Fx*cos(psi(k)) - Fy*sin(psi(k)));
   opti.subject_to(Fpy(k) == Fx*sin(psi(k)) + Fy*cos(psi(k)));
   
   % control  forces
   opti.subject_to(Fcx(k) == Fpx(k)*cos(theta) + Fpy(k)*sin(theta));
   opti.subject_to(Fcy(k) == -Fpx(k)*sin(theta) + Fpy(k)*cos(theta));
   ...
   k3 = Fcy(k)+dt/2*k2;
   k4 = Fcy(k)+dt*k3;
   Ecy_next = Ecy(k) + dt/6*(k1+2*k2+2*k3+k4); 
   opti.subject_to(Ecy(k+1)==Ecy_next); % close the gaps
...
prob1.Fpx = opti.debug.value(Fpx); prob1.Fpy = opti.debug.value(Fpy);
prob1.Fcx = opti.debug.value(Fcx); prob1.Fcy = opti.debug.value(Fcy);
...
prob1.Fpx = sol.value(Fpx); prob1.Fpy = sol.value(Fpy);
prob1.Fcx = sol.value(Fcx); prob1.Fcy = sol.value(Fcy);
...
nexttile;
plot(prob1.tvec(2:end),prob1.Fpx); box off;
ylabel('$F_{X(p)}$ [N]','Interpreter','latex');
xlabel('$t$ [s]','Interpreter','latex');
yline(Dxf*2 + Dxr*2,'LineStyle','--','DisplayName','Fxf (lim)')
yline(-(Dxf*2 + Dxr*2),'LineStyle','--','DisplayName','Fxf (lim)')

nexttile;
plot(prob1.tvec(2:end),prob1.Fpy); box off;
ylabel('$F_{Y(p)}$ [N]','Interpreter','latex');
xlabel('$t$ [s]','Interpreter','latex');
yline(Dyf*2 + Dyr*2,'LineStyle','--','DisplayName','Fxf (lim)')
yline(-(Dyf*2 + Dyr*2),'LineStyle','--','DisplayName','Fxf (lim)')

nexttile;
plot(prob1.tvec(2:end),prob1.Fcx); box off;
ylabel('$F_{X(c)}$ [N]','Interpreter','latex');
xlabel('$t$ [s]','Interpreter','latex');
yline(Dxf*2 + Dxr*2,'LineStyle','--','DisplayName','Fxf (lim)')
yline(-(Dxf*2 + Dxr*2),'LineStyle','--','DisplayName','Fxf (lim)')

nexttile;
plot(prob1.tvec(2:end),prob1.Fcy); box off;
ylabel('$F_{Y(c)}$ [N]','Interpreter','latex');
xlabel('$t$ [s]','Interpreter','latex');
yline(Dyf*2 + Dyr*2,'LineStyle','--','DisplayName','Fxf (lim)')
yline(-(Dyf*2 + Dyr*2),'LineStyle','--','DisplayName','Fxf (lim)')
...
% combined slip
Fy1 = Fy01.*sqrt(1.01 - (Fxfval/Dxf).^2);
Fy2 = Fy02.*sqrt(1.01 - (Fxfval/Dxf).^2);
Fy3 = Fy03.*sqrt(1.01 - (Fxrval/Dxr).^2);
Fy4 = Fy04.*sqrt(1.01 - (Fxrval/Dxr).^2);

% the total forces and moments acting on the vehicle
% longitudanal force
F_X1 = Fxfval.*cos(delta) - Fy1.*sin(delta);
F_X2 = Fxfval.*cos(delta) - Fy2.*sin(delta);
F_X3 = Fxrval;
F_X4 = Fxrval;
F_X = F_X1 + F_X2 + F_X3 + F_X4;

% lateral force
F_Y1 = Fy1.*cos(delta) + Fxfval.*sin(delta);
F_Y2 = Fy2.*cos(delta) + Fxfval.*sin(delta);
F_Y3 = Fy3;
F_Y4 = Fy4;
F_Y = F_Y1 + F_Y2 + F_Y3 + F_Y4;

% total moment 
M_Z1 = Fy1.*(lx1*cos(delta) + ly1*sin(delta)) - Fxfval.*(ly1*cos(delta) - lx1*sin(delta));
M_Z2 = Fy2.*(lx2*cos(delta) + ly2*sin(delta)) - Fxfval.*(ly2*cos(delta) - lx2*sin(delta));
M_Z3 = Fy3*lx3 - Fxrval*ly3;
M_Z4 = Fy4*lx4 - Fxrval*ly4;
